(function(){
  ...
  let globalData={version:1,sections:null,settings:null,updated_at:null};
  ...
  function applyRemote(){
    ...
    if(globalData.settings&&typeof globalData.settings==='object'){
      settings=Object.assign({},globalData.settings);
  ...
  async function pushGlobal(reason){
    ...
    try{
      ...
      if(sha) body.sha=sha;
      const put=await fetch(`https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/site-data.json`,{
        method:'PUT',
      ...
      console.error('Shared site data sync failed:',e);
      if(typeof showToast==='function')showToast('Ошибка сохранения на GitHub');
      return false;
    }finally{
      syncing=false;
      if(queued){queued=false;setTimeout(()=>pushGlobal('Save queued admin changes'),0)}
    }
  }
  ...
  function patchSetSetting(){
    ...
    const wrapped=async function(key,value){
      ...
      if(globalLoaded){
        ...
        return pushGlobal('Save site setting: '+key);
      }
      return ready.then(()=>pushGlobal('Save site setting: '+key));
    };
    wrapped.__persistence=true;
    window.setSetting=wrapped;
  }
  ...
  function patchDeleteSection(){
    ...
    const wrapped=async function(id){
      ...
      await pushGlobal('Delete site section from admin');
      return result;
    };
    wrapped.__persistence=true;
    window.deleteSection=wrapped;
  }
  ...
  function patchAddSection(){
    ...
    const wrapped=async function(){
      ...
      await pushGlobal('Add site section from admin');
      return result;
    };
    wrapped.__persistence=true;
    window.addSection=wrapped;
  }
  ...
  function patchMagazineWorks(){
    ...
    const wrapped=function(key,value){
      const result=originalSave(key,value);
      if(key==='magazine_works'&&globalLoaded){
        globalData.magazine_works=Array.isArray(value)?value:[];
        pushGlobal('Save magazine contest works').catch(()=>{});
      }
      return result;
    };
    wrapped.__persistence=true;
    window.saveToCache=wrapped;
  }
